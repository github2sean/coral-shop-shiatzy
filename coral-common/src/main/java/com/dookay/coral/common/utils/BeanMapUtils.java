...
public final class BeanMapUtils {
	 ...
	 * @throws InvocationTargetException failed to call setters
	 */
	public static Map<String, String> toMap(Object bean)throws IntrospectionException, IllegalAccessException, InvocationTargetException {
		...
		try {
			...
			for (int i = 0; i < descriptors.length; i++) {
				...
				if (!"class".equals(name)) {
					params.put(name, propertyUtilsBean.getNestedProperty(obj, name) != null ? propertyUtilsBean.getNestedProperty(obj, name).toString():"");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
