...
<div class="container">
    <div class="do-list-header">
        <a href="javascript:;" class="link-down font-14 j_panel_trigger"
           data-panel="j_panel_cat">${web:selectLanguage()=='en_US'?goodsCategoryDomain.enName:goodsCategoryDomain.name}</a>
        <div class="pull-right font-14">
            <a href="javascript:;" class="link-down j_panel_trigger" data-panel="j_panel_filter"><spring:message
                    code="goods.list.filter"/></a>
            <a href="javascript:;" class="link-down j_panel_trigger" data-panel="j_panel_sort"><spring:message
                    code="goods.list.sort"/></a>
        </div>
    </div>
        ...
        <c:forEach var="goods" items="${goodsDomainPageList.list}" varStatus="num">
            ...
            <li>
                <a href="/goods/details/${firstItem.id}">
                    ...
                    <p><span class="do-pro-price <c:if test="${not empty firstItem.discountPrice}">xzc-price</c:if>"
                             name="goodsPrice" data-value="${firstItem.price}">${firstItem.price}</span>
                        <c:if test="${not empty  firstItem.discountPrice }">
                            <span class="do-pro-price xzc-dis-price"
                                  data-value="${firstItem.discountPrice}">${firstItem.discountPrice}</span>
                        </c:if>
                    </p>
                ...
                <!--Todo:收藏按钮-->
                <i class="icon-collect j_collect hide" data-value="${firstItem.id}"
                   data-ids="${goods.sizeDomainList[0].id}">
                    <svg class="do-heart">
...
<div id="j_panel_cat" class="pro-filter-panel panel-cat">
    ...
    <ul class="do-sort-list">
        <c:forEach var="item" items="${categoryList}">
            <li><a href="/goods/list?categoryId=${item.id}">${web:selectLanguage()=='en_US'?item.enName:item.name}</a>
            </li>
        </c:forEach>
    </ul>
</div>
<div id="j_panel_filter" class="pro-filter-panel panel-filter">
    ...
    <form action="" class="filterForm">
        ...
        <div class="do-sort link-down"><spring:message code="goods.list.filter"/>
            <button type="reset" class="btn-reset"><spring:message code="goods.list.reset"/></button>
        </div>
        ...
        <div class="text-center">
            <button type="button" class="btn-submit"><spring:message code="subscription.complete"/></button>
        </div>
    </form>
</div>
